...
**There are two signup CTAs and they share one submit path.** `NewsletterSignup.astro` is the dismissible popup, rendered site-wide by `BaseLayout.astro`; `NewsletterInline.astro` is the end-of-post CTA, rendered by `BlogLayout.astro` after the `<slot />`, so **every blog post gets one automatically — never write a signup CTA into a post's Markdown**. Both call `bindNewsletterForm` in `src/scripts/newsletter-signup.js`, which owns the endpoint contract, the `cc-newsletter-*` bookkeeping and the error copy; only the presentation around it differs. Keep new CTAs on that function rather than re-implementing the fetch. The two suppression rules are deliberately different: `isSignupSuppressed` (popup) counts a 30-day dismissal, `signupCompletionState` (inline) does not — dismissing the popup silences the popup, not every mention of the newsletter. Each CTA's markup needs unique element ids and its own `[data-newsletter-status]`, and tags its `dataLayer` events with `newsletter_source` (`popup` / `in_post`). A successful submit broadcasts `cc:newsletter-pending` on `document` so the other mounted CTA stands down — without it, a page shows a live second form directly under "check your inbox".

The inline CTA is server-rendered and always visible, so it submits even when the client bundle never runs. The signup POST therefore **303s to `/newsletter/pending/` (or `/newsletter/error/`) when the request prefers `text/html`**, and keeps its JSON response for everything else — the fetch path sends `Accept: application/json`, and a caller sending no Accept at all still gets JSON. `/newsletter/pending/` writes the same `cc-newsletter-*` keys the fetch path records, so the CTAs stand down for a no-JS visitor too. `/newsletter/error/` renders the inline CTA for the same reason: it is the retry path a no-JS visitor is sent to, and the popup — hidden until a script reveals it — cannot be one. Popup analytics distinguish the two exits: `newsletter_popup_dismissed` is the visitor closing it, `newsletter_popup_stood_down` is the coordinated retreat after an inline signup.

`src/components/marketing/NewsletterSignup.astro` posts to `/api/forms/newsletter` (`functions/packages/forms/newsletter/newsletter.js`) and runs double opt-in: the POST only sends a signed, two-hour confirmation link. The link is a GET that renders a page which immediately submits the confirming POST via inline script (the button stays as a no-JS fallback), so a person is subscribed in one click while an email security scanner — which fetches every link but does not run scripts — cannot confirm anyone. The Resend Contact is only ever created by that POST. It needs two more secrets on the `api` component: `RESEND_CONTACTS_API_KEY` (a Resend key with Contacts access — the send-only `RESEND_API_KEY` still delivers the confirmation email) and `NEWSLETTER_CONFIRM_SECRET` (32+ random bytes used to sign links; rotating it invalidates every link in flight).
...
Every confirmed subscriber is placed in the Resend segment **"Concrete Comeback"** (`35b5b4f0-ac74-4ffa-b7d3-620e4f0014ed`, hard-coded as `DEFAULT_SEGMENT_ID` in `newsletter.js`, overridable with `NEWSLETTER_SEGMENT_ID`), because the Resend team is shared with other sites and account-wide contacts include their lists too. Read the subscriber count from that segment, not from total contacts. The ID is deliberately not a `${VAR}` in `functions/project.yml`: an unresolved name there aborts the whole `forms` deploy. New contacts get the segment in the create call; returning contacts get a follow-up add-to-segment call whose failure is logged but does not block the confirmation.
